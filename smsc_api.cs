...
public class SMSC
{
	...
	//const string SMSC_PASSWORD = "73e891c2960c51aca8e59cf1b2f19fc0";	// пароль или MD5-хеш пароля в нижнем регистре
	...
	public string[] send_sms(string phones, string message, int translit = 0, string time = "", int id = 0, int format = 0, string sender = "", string query = "", string[] files = null)
	{
        ...
        if (SMSC_DEBUG)
        {
            if (Convert.ToInt32(m[1]) > 0)
            {
                ...
                MessageBox.Show("SMS отправлена");
            }
            else
            {
                string err = "Ошибка №" + m[1].Substring(1, 1) + (m[0] != "0" ? ", ID: " + m[0] : "");
                _print_debug(err);
                MessageBox.Show(err);
            }
		}
